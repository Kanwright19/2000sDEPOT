...
function Auth({ setUser }) {
    ...
    const signUpSchema = yup.object().shape({
        first_name: yup.string().min(4,' Name is too short!').max(15, 'Name is too long'),
        last_name:yup.string().min(4, 'Last Name is too short!').max(30, 'Last Name is too long'),
    ...
    return (
        <Container className="auth-container d-flex flex-column">
            <Button onClick={toggleSignUp} variant="primary">
                {signUp ? 'Please login to see games' : 'Signup to acess forum!'}
            </Button>
